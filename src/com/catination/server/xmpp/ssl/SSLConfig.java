 ...
 */
package com.catination.server.xmpp.ssl;
...
public class SSLConfig {
	...
	static {
		...
		log.debug("keyStoreLocation1=" + keyStoreLocation);
		keyStoreLocation = classPath.getPath()
				//+ File.separator
				+ keyStoreLocation;
		keyPass = Config.getString("xmpp.ssl.keypass", "zhqh99");
		
		trustStoreLocation = Config.getString("xmpp.ssl.truststore", "conf"
				+ File.separator + "security" + File.separator + "truststore");
		log.debug("trustStoreLocation1=" + trustStoreLocation);
		trustStoreLocation = classPath.getPath()
				//+ File.separator
				+ trustStoreLocation;
		trustPass = Config.getString("xmpp.ssl.trustpass", "changeit");

		log.debug("keyStoreLocation2=" + keyStoreLocation);
		log.debug("trustStoreLocation2=" + trustStoreLocation);
		...
		try {
			...
			keyStore.load(new FileInputStream(keyStoreLocation), keyPass
					.toCharArray());
			log.debug("storeType="+storeType+", keyStore=" + keyStore+", keyPass=" + keyPass);
		} catch (Exception e) {
			log.error(
		...
		try {
			...
			trustStore.load(new FileInputStream(trustStoreLocation), trustPass
					.toCharArray());
			log.debug("storeType="+storeType+", trustStore=" + trustStore+", trustPass=" + trustPass);
	...
	public static KeyStore getKeyStore() throws IOException {
		log.debug("getKeyStore(), keyStore="+keyStore);
		if (keyStore == null) {
