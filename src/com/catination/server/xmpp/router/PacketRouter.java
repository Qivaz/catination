...
public class PacketRouter {
    ...
    private HBRouter hbRouter;

    /**
     * Constructor. 
     */
    public PacketRouter() {
        messageRouter = new MsgRouter();
        presenceRouter = new PresenceRouter();
        iqRouter = new IQRouter();
        hbRouter = new HBRouter();
    }
    ...
    public void route(Packet packet) {
        if (packet instanceof Message) {
            ...
            route((IQ) packet);
        } else if (packet instanceof HeartBeatMessage) {
            route((HeartBeatMessage) packet);
        } else {
    ...
    /**
     * Routes the HeartBeat packet.
     * 
     * @param packet the packet to route
     */
    public void route(HeartBeatMessage packet) {
        hbRouter.route(packet);
    }
}
