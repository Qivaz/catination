...
import com.catination.server.xmpp.router.PacketRouter;
...
public class StanzaHandler {
	...
	public void process(String stanza, XMPPPacketReader reader)
			throws Exception {
		log.debug("StanzaHandler.process(), stanza="+stanza);
	...
	private void processPresence(Element doc) {
		...
		Presence packet;
		/*
		 * <presence id="ih73Y-19" type="unavailable"></presence>
		 */
		try {
	...
	private void processHB(Element doc) {
		log.debug("processHB()..."+doc);
		Element eLocation = doc.element("location");
		log.debug("processHB()..."+eLocation.asXML());
		Element eCdma = eLocation.element("cdma");
		Element eGsm = eLocation.element("gsm");
		Element eLon = eLocation.element("lon");
		Element eLat = eLocation.element("lat");

		HeartBeatMessage packet = null;
		if (eCdma != null) {
			Element eSid = eCdma.element("sid");
			Element eNid = eCdma.element("nid");
			if (eSid == null || eNid == null) {
				packet = new HeartBeatMessage(doc, true, 99, 99);
			} else {
				packet = new HeartBeatMessage(doc, true,
						Integer.valueOf(eSid.getText()),
						Integer.valueOf(eNid.getText()));
			}
		} else if (eGsm != null) {
			Element eLac = eGsm.element("lac");
			Element eCid = eGsm.element("cid");

			if (eLac == null || eCid == null) {
				packet = new HeartBeatMessage(doc, false, 99, 99);
			} else {
				packet = new HeartBeatMessage(doc, false,
						Integer.valueOf(eLac.getText()),
						Integer.valueOf(eCid.getText()));
			}
		} else if (eLon != null && eLat != null) {
			double lon = Double.valueOf(eLon.getText());
			double lat = Double.valueOf(eLat.getText());
			packet = new HeartBeatMessage(doc, lon, lat);
		}

		log.debug("processHB(), Session.address="+session.getAddress()+", packet="+packet);
		if (packet != null) {
			packet.setFrom(session.getAddress());
			router.route(packet);
		}
	...
	private void processIQ(Element doc) {
		...
		log.debug("processIQ(), Session.address="+session.getAddress());
		packet.setFrom(session.getAddress());
		router.route(packet);
		session.incrementClientPacketCount();
	}
	...
	private void startTLS() throws Exception {
		...
		log.info("StanzaHandler.startTLS(), policy="+policy);
