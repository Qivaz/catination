        ...
        try {
          ...
          //  locateServer();
            serverName = Config.getString("xmpp.domain", "127.0.0.1")
                    .toLowerCase();
            log.info("XmppServer.start(), serverName="+serverName);
            context = new ClassPathXmlApplicationContext("springContextConfig.xml");
...
//            }
            ...
            log.info("Catination Server v" + version);
            
            SSLConfig sslc = SSLConfig.getInstance();
        } catch (Exception e) {
        	log.error("Spring Configuration loading failed!!!");
            e.printStackTrace();
            shutdownServer();
        }
    }
    ...
    public Object getBean(String beanName) {
    	log.info("XmppServer.getBean(), beanName="+beanName);
    	/*try {
    		throw new Exception();
    	} catch (Exception e) {
    		e.printStackTrace();
    ...
    public boolean isStandAlone() {
        ...
        try {
            standalone = Class
                    .forName("com.catination.server.starter.ServerStarter") != null;
