...
public abstract class FileUploadBase {
    ...
    public List<FileItem> parseRequest(RequestContext ctx)
        ...
        try {
            ...
            log.info("parseRequest(), iter="+iter+", .hasNext()="+iter.hasNext());
            while (iter.hasNext()) {
                ...
                log.info("parseRequest(), "+item.getFieldName()+", "+item.getContentType()+", "+
                                                   item.isFormField()+", "+fileName);
                items.add(fileItem);
                try {
    ...
    private class FileItemIteratorImpl implements FileItemIterator {
        ...
        private boolean findNextItem() throws IOException {
            ...
            //log.debug("findNextItem(), multi="+multi);
            //log.debug("findNextItem(), multi.readByte()="+multi.readByte());
            //log.debug("findNextItem(), multi.readHeaders()="+multi.readHeaders());
            for (;;) {
                boolean nextPart;
                if (skipPreamble) {
        ...
        public boolean hasNext() throws FileUploadException, IOException {
            if (eof) {
            	log.debug("hasNext(), eof");
                return false;
            }
            if (itemValid) {
            	log.debug("hasNext(), itemValid");
                return true;
            }
            try {
            	log.debug("hasNext(), else..");
                return findNextItem();
            } catch (FileUploadIOException e) {
                // unwrap encapsulated SizeException
                throw (FileUploadException) e.getCause();
